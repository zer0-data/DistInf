...
def load_model(
    ...
    if attn_type == 'dense':
        ...
        model = StarAttentionModel(
            ...
            # Block size is used for partitioning
...
def main(
    ...
    Args:
        ...
        attn_type: type of attention. One of ['dense', 'star']
        ...
        block_size: block size for star attention prefill. Default: -1 (use num_blocks)
        top_k: K value for token sampling in selection layers. Default: 256
        selection_layers: List of layer indices to perform token sampling. Default: [2, 14]
        num_blocks: Number of blocks to split context into for prefill. Default: 4
    ...
    with open(output_file, output_file_mode, encoding='utf-8', buffering=1) as fout:
        for input_sample in tqdm(input_data, total=len(input_data)):
            # Set the sequence context for the model's history tracker
            # This is done on all ranks
            model.model.model.set_sequence_context(
                input_ids=None,
                input_text=input_sample['input_context'] + " " + input_sample['input_query']
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--attn_type', required=True, help="type of attention ('dense' or 'star')")
    parser.add_argument('--block_size', type=int, default=-1, help='block size for star attention (overrides num_blocks if > 0)')
    
    # New arguments for Tidal-Prefill
    parser.add_argument('--top_k', type=int, default=256, help='K value for token sampling in selection layers')
    parser.add_argument('--selection_layers', type=str, default='2,14', help='Comma-separated list of layer indices for sampling (e.g., "2,14")')
    parser.add_argument('--num_blocks', type=int, default=4, help='Number of blocks for sequential prefill (if block_size is not set)')
    ...
    # Parse selection_layers
    selection_layers = [int(x.strip()) for x in args.selection_layers.split(',') if x.strip()]
