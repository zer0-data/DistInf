...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # --- MODIFICATION ---
        # The attention_interface (llama_tidal_attention_forward) can now return
        # (attn_output, attn_weights, chosen_tokens)
        # We capture all three return values.
        
        # We pass **kwargs, which will include 'selection_layers'
        attn_output_tuple = attention_interface(
            self,
            query_states,
            key_states,
            value_states,
        ...
        attn_output = attn_output_tuple[0]
        attn_weights = attn_output_tuple[1]
        chosen_tokens = attn_output_tuple[2] if len(attn_output_tuple) == 3 else None

        attn_output = attn_output.reshape(*input_shape, -1).contiguous()
        attn_output = self.o_proj(attn_output)
        
        # Return chosen_tokens if they exist
        if chosen_tokens is not None:
            return attn_output, attn_weights, chosen_tokens
        else:
            return attn_output, attn_weights
...
class LlamaDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        top_token_indices = None

        # Self Attention
        # **kwargs (containing selection_layers) is passed to self_attn
        ...
        # Logic to handle 3-tuple return (attn_output, attn_weights, chosen_tokens)
        if len(attn_output) == 3:
            hidden_states, self_attn_weights, top_token_indices = attn_output
        else:
        ...
        if top_token_indices is not None:
            # Return sampled tokens up the stack
            return (outputs, top_token_indices)
        else:
            return (outputs,)
...
class LlamaModel(LlamaPreTrainedModel):
    def __init__(self, config: LlamaConfig):
        ...
        # Global dictionary to store top tokens for each layer at each generation step
        # Structure: {generation_step: {layer_idx: top_tokens}}
        self.global_top_tokens_history = {}

        # Current sequence identifier (will be set when processing begins)
        self.current_sequence_id = None
        self.current_input_text = None

        # Optional tokenizer reference for text decoding
    ...
    def set_tokenizer_for_decode(self, tokenizer):
        """
        Set the tokenizer reference for automatic text decoding when input_text is not provided.
    ...
    def _generate_sequence_id(self, input_ids, input_text=None):
        ...
        if input_ids is not None:
            # Take only the original prompt tokens (before generation)
            # We use the full input_ids for the initial tokens
        ...
        elif input_ids is not None and hasattr(self, "_tokenizer_for_decode"):
            # Try to decode input_ids to get a meaningful text representation
            try:
                decoded_text = self._tokenizer_for_decode.decode(
                    input_ids[0] if len(input_ids.shape) > 1 else input_ids,
                    skip_special_tokens=True,
                )
                # Limit the length to avoid extremely long decoded texts
                ...
                # Fallback to a placeholder if decoding fails
                self.current_input_text = f"<Decoded from {len(input_tokens)} tokens>"
        else:
            # Set a placeholder if no text can be determined
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        prefill_block_idx: Optional[int] = None, # <-- NEW ARGUMENT
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Set sequence context if not already set (for automatic sequence identification)
        if self.current_sequence_id is None and input_ids is not None:
            self.set_sequence_context(input_ids)
        ...
        # --- MODIFIED LOGIC ---
        # Determine the key for history tracking
        if prefill_block_idx is not None:
            # We are in sequential prefill, use the block index as the step
            current_generation_step = prefill_block_idx
        else:
            # We are in standard decoding, use the last token position
        ...
        # Initialize the dictionary for this generation step if it doesn't exist
        if current_generation_step not in self.global_top_tokens_history:
            self.global_top_tokens_history[current_generation_step] = {}

        for idx, decoder_layer in enumerate(
            self.layers[: self.config.num_hidden_layers]
        ):
            decoder_layer.self_attn.pos_mask = prev_mask
            decoder_layer.self_attn.pos_index = prev_index
            top_tokens_for_layer = None

            if output_hidden_states:
                all_hidden_states += (hidden_states,)
            
            # Pass **flash_attn_kwargs (containing selection_layers) down
            ...
            # Check for (outputs, top_token_indices)
            if len(decoder_layer_output) == 1:
                layer_outputs = decoder_layer_output[0]
            else:
                layer_outputs, top_tokens_for_layer = decoder_layer_output

            hidden_states = layer_outputs[0]
            if top_tokens_for_layer is not None:
                # Store top tokens for this layer at this generation step
                self.global_top_tokens_history[current_generation_step][
                    idx
                ] = top_tokens_for_layer

            if output_attentions:
                all_self_attns += (layer_outputs[1],)

            prev_mask = decoder_layer.self_attn.pos_mask
            prev_index = decoder_layer.self_attn.pos_index
...
# [Continuing from Part 1 of llama_tidaldecoding (2).py]
# ... (LlamaModel class ends)
...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    # --- NEW METHODS ---
    # Delegating history-tracking methods to the inner model
    ...
    def set_sequence_context(self, input_ids, input_text=None):
        ...
        Args:
            ...
            input_text (str, optional): The original input text
        """
        return self.model.set_sequence_context(input_ids, input_text)

    def save_top_tokens_history(self, output_dir="./top_tokens_logs", filename=None):
        """
        Save the complete top tokens history to a JSON file.
        Delegates to the underlying model.
        ...
        Returns:
            str: Path to the saved file
        """
        return self.model.save_top_tokens_history(output_dir, filename)

    def get_top_tokens_history(self):
        """
        Get the complete history of top tokens.
        Delegates to the underlying model.

        Returns:
            dict: {generation_step: {layer_idx: top_tokens}}
        """
        return self.model.get_top_tokens_history()
        
    def get_top_tokens_for_step(self, generation_step):
        """
        Get top tokens for a specific step.
        Delegates to the underlying model.
        """
        return self.model.get_top_tokens_for_step(generation_step)

    def clear_top_tokens_history(self):
        """
        Clear the global top tokens history.
        Delegates to the underlying model.
        """
        return self.model.clear_top_tokens_history()
    ...
    def set_tokenizer_for_decode(self, tokenizer):
        ...
        Args:
            tokenizer: The tokenizer instance used for decoding input_ids to text
        """
        return self.model.set_tokenizer_for_decode(tokenizer)
    # --- END NEW METHODS ---
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM], # <-- MODIFIED to accept **kwargs
    ) -> CausalLMOutputWithPast:
        r"""
        ...
        # **kwargs (containing selection_layers and prefill_block_idx)
