...
from .utils import flatten_varlen_lse, unflatten_varlen_lse, update_out_and_lse


def _star_flash_attn_varlen_forward(
    q,
    k,
    v,
    cu_seqlens_q,
    ...
    block_lse = flatten_varlen_lse(block_lse, cu_seqlens=cu_seqlens_q)
    ...
    dist.all_gather(lse_gather, block_lse, async_op=False)
    dist.barrier()
...
class StarFlashAttnVarlenFunc(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        q,
        ...
        out, softmax_lse = _star_flash_attn_varlen_forward(
            q,
            k,
            v,
            cu_seqlens_q,
...
def star_flash_attn_varlen_func(
    q,
    k,
    v,
    cu_seqlens_q,
    ...
    """
    return StarFlashAttnVarlenFunc.apply(
        q,
        k,
        v,
        cu_seqlens_q,
